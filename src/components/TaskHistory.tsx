...
const TaskHistory: React.FC<TaskHistoryProps> = ({ tasks }) => {
  ...
  return (
    <div className="mt-12 w-full max-w-2xl mx-auto">
      <h3 className="text-white text-sm font-bold mb-4 uppercase tracking-wider text-left">
        SESSION HISTORY
      </h3>
      <div className="space-y-2">
        {visibleTasks.map((task, index) => (
          <div
            key={task.id}
            className={`flex items-center justify-between p-3 bg-gray-900 border border-gray-800 rounded transition-opacity duration-1000 ${
